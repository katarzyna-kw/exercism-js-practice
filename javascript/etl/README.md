const transform = (old) => {
  ...
  for( let s in old) {
	  for( let c of old[s]){
		  ...
		  result[c.toLowerCase()]=parseInt(s);
	  }
  }
  return result;
};
...
(Typically, this is followed by, "We're only going to need to run this once." That's then typically followed by much forehead slapping and
