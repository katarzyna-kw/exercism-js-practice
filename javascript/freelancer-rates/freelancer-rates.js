...
const DAILY_RATE_RULE = 8
const BILLABLE_DAYS_PER_MONTH = 22

export function dayRate(ratePerHour) {
  return ratePerHour*DAILY_RATE_RULE;
}
...
export function monthRate(ratePerHour, discount) {
  const withoutDiscount=(dayRate(ratePerHour) * BILLABLE_DAYS_PER_MONTH)
  const discountTotal=withoutDiscount*discount
  return Math.ceil(withoutDiscount-discountTotal);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  const dailyRate=monthRate(ratePerHour, discount)/BILLABLE_DAYS_PER_MONTH
  ;
